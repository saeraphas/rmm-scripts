...
$MessageText = "Hardware Readiness Check complete, result is $($outObject.returnResult)."
Write-Output $MessageText | Out-File -FilePath $Win11ManualUpgradeLogPath -Append
...
Switch ($outObject.returnCode) {
    0 {    
        ...
        If (-not $Win11UpgradeUtilityExists) {
            try {
                ...
                $MessageText = "Downloading Upgrade Utility from $Win11UpgradeURL failed."
                Write-Error $MessageText
                Write-Output $MessageText | Out-File -FilePath $Win11ManualUpgradeLogPath -Append
            }
        }
        ...
        If ($Win11UpgradeUtilityExists) {
            $MessageText = "Downloading Upgrade Utility from $Win11UpgradeURL completed."
            Write-Output $MessageText | Out-File -FilePath $Win11ManualUpgradeLogPath -Append
            try {
                $MessageText = "Starting Upgrade Utility from $UpgradePath completed."
                Write-Output $MessageText | Out-File -FilePath $Win11ManualUpgradeLogPath -Append
                Start-Process -FilePath $UpgradePath -ArgumentList "/QuietInstall /SkipEULA /Auto Upgrade /NoRestartUI"        
            }
            catch {
                $MessageText = "Starting Upgrade Utility from $UpgradePath failed."
                Write-Error $MessageText
                Write-Output $MessageText | Out-File -FilePath $Win11ManualUpgradeLogPath -Append
            }
        }
    ...
    default { $outObject | ConvertTo-Json -Compress | Out-File -FilePath $Win11ManualUpgradeLogPath -Append }
}
